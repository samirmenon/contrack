      ...
      for (k = 1; k <= i__1; ++k) {
        ...
        for (i = 0; i <= i__2; ++i) {
          if (i == 0) {
            ...
            bb(n+1) += cf(i+1) * (a(i - 1+1) + a(i + 1+1)) / 2.0;
          }
          %/* L55: */
          b(n - 1+1) += cf(i+1) * a(i+1);
        }
        if (k == 1) {
          fact0 = fact0 * (-1 / al) / k;
        } else {
        ...
        fact = fact0 * be;
        termb += e(k+1) * (b(k - 1+1) * fact0 * k + bb(k+1) * fact);
        terml -= k * e(k+1) * b(k - 1+1) * fact / al;
    ...
    if (level(0+1) != 0) {
      ...
      for (k = 1; k <= i__1; ++k) {
        ...
        for (i = 0; i <= i__2; ++i) {
          if (i > 0) {
            i__3 = i - 1;
            dd(n+1) += i * cf(i+1) * e(i+1) * e(n - i+1) * pow(de, i__3);
          }
          %/* L46: */
          d(n - 1+1) += cf(i+1) * e(i+1) * e(n - i+1) * pow(de, i);
        }
        fact = -fact * (-0.5 - k + 1) / k / al;
        termd += fact * dd(k+1);
        terml -= fact * d(k - 1+1) * k / al;
        add0 = add;
        add = fact * d(k - 1+1);
        term += add;
