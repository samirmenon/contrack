function [ logBhamScore ] = ctrLogBinghamScore(t, D, C, pAngle)
...
% logbhamScore: log(C(σ3, σ2))  - (v3'*t / sin(σ3))^2 - (v2'*t / sin(σ2))^2 
%               σi = σm + σi* 
%               σi* = λi' / (λi + λi') * δ
...
% Compute the log Bingham score (not a probability).
logBhamScore =  - (t3*t3) - (t2*t2);

%Convert the score to a log-probability
logBhamScore = log(C) + logBhamScore + log(dar);
end
