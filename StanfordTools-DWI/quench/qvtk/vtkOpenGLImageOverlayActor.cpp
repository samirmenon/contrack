/*=========================================================================
  ...
  Module:    vtkOpenGLImageOverlayActor.cxx
...
vtkStandardNewMacro(vtkOpenGLImageOverlayActor);
#endif

// Initializes an instance, generates a unique index.
vtkOpenGLImageOverlayActor::vtkOpenGLImageOverlayActor()
{
  this->Index = 0;
  this->RenderWindow = 0;
...
void vtkOpenGLImageOverlayActor::ReleaseGraphicsResources(vtkWindow *renWin)
{
...
void vtkOpenGLImageOverlayActor::SetQuadExtent(double extent[6])
{
  int idx, modified = 0;
  
  for (idx = 0; idx < 6; ++idx)
    {
    if (this->QuadExtent[idx] != extent[idx])
      {
      this->QuadExtent[idx] = extent[idx];
      modified = 1;
...
//----------------------------------------------------------------------------
void vtkOpenGLImageOverlayActor::SetQuadExtent(double minX, double maxX, 
                                     ...
                                     double minZ, double maxZ)
{
  ...
  this->SetQuadExtent(extent);
}

unsigned char *vtkOpenGLImageOverlayActor::MakeDataSuitable(int &xsize, int &ysize,
                                                     int &release,
                                                     int &reuseTexture)
{
  int contiguous = 0;
  ...
  this->Coords[0] = this->QuadExtent[0];
  this->Coords[1] = this->QuadExtent[2];
  this->Coords[2] = this->QuadExtent[4];
  
  this->Coords[3] = this->QuadExtent[1];
  this->Coords[4] = this->QuadExtent[2 + (xdim == 1)];
  this->Coords[5] = this->QuadExtent[4];
  
  this->Coords[6] = this->QuadExtent[1];
  this->Coords[7] = this->QuadExtent[3];
  this->Coords[8] = this->QuadExtent[5];
  
  this->Coords[9] = this->QuadExtent[0];
  this->Coords[10]= this->QuadExtent[2 + (ydim == 1)];
  this->Coords[11]= this->QuadExtent[5];
...
void vtkOpenGLImageOverlayActor::Load(vtkRenderer *ren)
{
...
// video card
int vtkOpenGLImageOverlayActor::TextureSizeOK( int size[2] )
{
...
void vtkOpenGLImageOverlayActor::Render(vtkRenderer *ren)
{
...
// be fine)
void vtkOpenGLImageOverlayActor::InternalRender(vtkRenderer *ren)
{
  // for picking
  glDepthMask (GL_TRUE);
...
void vtkOpenGLImageOverlayActor::PrintSelf(ostream& os, vtkIndent indent)
{
