...
public:
    ...
    void getNearestPoint(const KdPoint &target,
                         int *bestIndex, float *bestSqDistance);
    float getSqDistance(const KdPoint &target);
protected:
    KdNode() {};
...
public:
    KdTree(Real destPoints[][3], int numDestPoints);
    ~KdTree();
    void getNearestPoint(const KdPoint &target,
                         int *bestIndex, float *bestSqDistance);
private:
 ...
 */
float KdNode::getSqDistance(const KdPoint &target)
{
    float sqDistance = 0;
    for (int ii = 0; ii < 3; ii++) {
        float delta;
        if (corners[0][ii] >= target[ii]) {
            delta = (float)corners[0][ii] - target[ii];
        } else if (corners[1][ii] <= target[ii]) {
            delta = (float)target[ii] - corners[1][ii];
        } else {
            delta = 0;
        }
...
void KdNode::expand()
{
    ...
    for (int axis = 0; axis < 3; axis++) {
        float tmpWidth = corners[1][axis] - corners[0][axis];
    ...
    // two sub-lists.
    //
    float axisMidpoint = corners[0][widestAxis] + axisWidth/2;
 ...
 */
KdTree::KdTree(Real destPoints[][3], int numDestPoints)
{
...
void KdTree::getNearestPoint(const KdPoint &target,
                             int *bestIndex, float *bestSqDistance)
{
    float deltaX = target[0] - pointArray[0].point[0];
    float deltaY = target[1] - pointArray[0].point[1];
    float deltaZ = target[2] - pointArray[0].point[2];
    *bestIndex = 0;
