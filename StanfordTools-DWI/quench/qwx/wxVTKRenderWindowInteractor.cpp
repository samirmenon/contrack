...
void wxVTKRenderWindowInteractor::OnKeyDown(wxKeyEvent &event)
{
  ...
  bCmdDwn = event.CmdDown();
#if !defined(__WXWINDOWS__) && !defined(__WXMAC__) && !defined(__WXCOCOA__)
  if(keycode == WXK_CONTROL)
	  bCmdDwn = true;
#endif
// #if (__WXMAC__)
//   // Ctrl-key should be set if Cmd is down on Macs all other platforms Command maps to Ctrl.
//   this->SetControlKey(event.CmdDown());
// #endif
}
//---------------------------------------------------------------------------
void wxVTKRenderWindowInteractor::OnKeyUp(wxKeyEvent &event)
{
 ...
 //  bool bCtrlDwn = event.ControlDown();  
// #if (__WXMAC__)
//   // Ctrl-key should be set if Cmd is down on Macs all other platforms Command maps to Ctrl.
//   bCtrlDwn = event.CmdDown();  
// #endif
