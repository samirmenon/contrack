...
// other necessary keyboard-dependent translations.
  EVT_KEY_DOWN    (wxVTKRenderWindowInteractor::OnKeyDown2)
...
wxVTKRenderWindowInteractor::wxVTKRenderWindowInteractor(wxWindow *parent,
      ...
      , UseCaptureMouse(0)
{
  this->GTKShiftPressedHack = false;
  this->RenderWindow = NULL;
  this->SetRenderWindow(vtkRenderWindow::New());
  this->RenderWindow->Delete();
  SetFocus();
...
void wxVTKRenderWindowInteractor::Initialize()
{
  ...
  // xxx dla enabled this. Why was it commented out by the wxVTK authors?
  RenderWindow->Start();
...
void wxVTKRenderWindowInteractor::UpdateSize(int x, int y)
{
  if( RenderWindow )
  {
    ...
    if ( x != Size[0] || y != Size[1] )
    {
      ...
      //      }
      //#endif
      //      cerr << "Onsize updating to (" << x << ", " << y << ")" << endl;
      //vtkRenderWindowInteractor::SetSize(x,y);
      //#endif
    }
  }
}
...
void wxVTKRenderWindowInteractor::OnMotion(wxMouseEvent &event)
{
 ...
 //   cerr << "eventY: " << event.GetY() << endl;
 //   cerr << "size: " << Size[1] << endl;
...
void wxVTKRenderWindowInteractor::OnKeyDown2(wxKeyEvent &event)
{
 if (event.GetKeyCode() == WXK_SHIFT) {
   GTKShiftPressedHack = true;
 }
 event.Skip();
}
...
void wxVTKRenderWindowInteractor::OnKeyDown(wxKeyEvent &event)
{
  ...
  // xxx dla ShiftDown is not reported correctly in WXGTK (at least for 
  // keyboard events...)

  bool shiftDown;
  #ifdef __WXGTK__
    shiftDown = GTKShiftPressedHack;
  #else
    shiftDown = event.ShiftDown();
  #endif

  // xxx dla
  // changed this to cmddown for mac support

  SetEventInformationFlipY(event.GetX(), event.GetY(), 
    event.CmdDown(), shiftDown, key, 0, NULL);
...
if (event.GetKeyCode() == WXK_SHIFT) {
  ...
  SetEventInformationFlipY(event.GetX(), event.GetY(), 
    event.CmdDown(), event.ShiftDown(), key, 0, NULL);
...
void wxVTKRenderWindowInteractor::OnButtonDown(wxMouseEvent &event)
{
  ...
  // xxx dla setting keyboard focus for window.
  SetFocus();
