...
/** Assumes that e1 is unit. Also assumes that the D tensor is
*  identity with diag : (1,e2,e3). i.e. eigvecs are unit vecs along 
*  x,y,z  */
...
double ctrFindIntegConstt(const double e2, const double e3, 
	const double dtheta, const double dphi)
{
	...
	for(double theta = -pi; theta<pi;theta+=dtheta) //From lower pole to upper pole
	{
	    for(double phi = 0; phi<2*pi; phi+=dphi) //Integrate over a disk sliced moving along the pole
	    {
	        ...
	        double bhscore = ctrBhamScore(t, e2, e3, 1);
...
int main()
{
	const double einit = 0.01, de=0.01, dtheta=0.001, dphi=dtheta;
...
#pragma omp parallel private(thread_id,e2,e3,dd)
	{
		thread_id = omp_get_thread_num();
		ss[thread_id]<<"bhamConst-t"<<thread_id<<"-"<<de<<"-"<<dtheta<<".txt";
		...
		for(e2=einit+ static_cast<double>(thread_id)*de;
			e2<1;e2+=nthreads*de){
			for(e3=einit;e3<e2;e3+=de){
				dd = ctrFindIntegConstt(e2,e3,dtheta,dphi);
				fprintf(fp[thread_id], "%lf %lf %lf\n", e2, e3, dd);
			}
			printf("\n t[%d], e2 : %lf",thread_id,e2);
